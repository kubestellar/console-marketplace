...
| 8 | Whether a completed `python-unit-tests.yml` / `ts-unit-tests.yml` run left a bounded, machine-readable record of test counts and coverage percentage | Every run's Summary tab shows this record | None today — only tool-native pytest/coverage/vitest console output | [`python-ts-unit-tests-ci-summary-gap.md`](./python-ts-unit-tests-ci-summary-gap.md) — **not yet met**: fix is a validated, ready-to-apply diff pair blocked on the same `workflows` permission gap as SLO 3 and SLO 7; see [issue #636](https://github.com/kubestellar/console-marketplace/issues/636) |
...
- **SLO 6** is the same undocumented-gap situation as SLO 5, for a different scheduled
  ...
  issue-filing step, or other notification on failure — the mechanism fix is now
  tracked together with SLO 5 under [issue #573](https://github.com/kubestellar/console-marketplace/issues/573)
  (the separate doc-only tracking issue #598 was closed once this row and
...
- **SLO 8** is the same class of gap as SLO 7, for `python-unit-tests.yml` and
  `ts-unit-tests.yml`: validated, ready-to-apply diffs exist for both workflows but
  cannot be pushed by automation for the same `workflows` App-permission reason —
  see [`python-ts-unit-tests-ci-summary-gap.md`](./python-ts-unit-tests-ci-summary-gap.md)
  for the preserved diffs and [issue #636](https://github.com/kubestellar/console-marketplace/issues/636).
  That runbook also independently confirms `python-unit-tests.yml` has been failing
  its coverage gate on every `main` run since at least 2026-09-09 — a separate,
  pre-existing coverage-gap issue, not itself an observability gap.
...
Do not mark SLO 2, SLO 3, SLO 5, SLO 6, SLO 7, or SLO 8 as met until the corresponding gap above is
