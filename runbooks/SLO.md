...
- **SLO 7** is the same class of gap as SLO 8, for `fuzz.yml` instead of
  `validate-json.yml`: the corpus-mutation/edge-case check logic has been extracted
  into a tested, standalone `scripts/fuzz_summary.py` (unit tests in
  `tests/test_fuzz_summary.py`, merged via PR #723), but wiring a call to it into
  `fuzz.yml`'s `fuzz-json` job needs the same `workflows` App permission automated
  PRs from this project do not carry — see
